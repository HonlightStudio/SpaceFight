...
public class EnemySystem : MonoBehaviour
{
    ...
    [SerializeField][Range(0,1)] private float enemySpeed;
    ...
    private void Update()
    {
        ...
        /*for (int i = 0; i < realYSize; i++)
        {
            ...
            {
                grid[i, j] = new Vector2(Mathf.SmoothStep(grid[i,j].x ,(-grid[0,0].x+target.x)+grid[i,j].x,enemySpeed),Mathf.SmoothStep(grid[i,j].y ,(-grid[0,0].y+target.y)+grid[i,j].y,enemySpeed));
                if (Mathf.Abs(grid[i, j].x - topRight.x) < 0.1f || Mathf.Abs(grid[i, j].x - bottomLeft.x) < 0.1f ||
                    Mathf.Abs(grid[i, j].y - topRight.y) < 0.1f)
                {
                    ChangeRandomTarget();
                }
            }
